...
use android_hid_connect::ai::{
    read_device_envelope, AiStats, DeviceEnvelope, FrameSummary,
};
...
fn enable_ai(stream: &mut TcpStream) -> Result<()> {
    let cfg = ControlMessage::AiConfig(AiConfig {
        flags: AI_FLAG_KEYFRAMES | AI_FLAG_FEATURES | AI_FLAG_MOTION | AI_FLAG_OBJECTS | AI_FLAG_TEXT,
    ...
    stream.flush()?;
    println!("  >> sent AiConfig: keyframes+features+motion+objects+text, 200ms interval, 64-D features");
    Ok(())
}

fn query_stats(stream: &mut TcpStream) -> Result<()> {
    let q = ControlMessage::AiQuery(AiQuery { since_timestamp_ms: 0 });
...
fn print_frame(s: &FrameSummary) {
    ...
    if !s.features.is_empty() {
        let preview: Vec<String> = s.features.iter().take(8).map(|f| format!("{:.2}", f)).collect();
        println!("         features[:8] = [{}]", preview.join(" "));
    }
    for v in s.motion.iter().take(3) {
        println!("         motion @({},{}) -> ({:+},{:+})", v.x, v.y, v.dx, v.dy);
    }
...
fn main() -> Result<()> {
    ...
    if mock_mode {
        ...
        print_stats(&s);
        println!("\n== Mock summary: {} frames synthesized in {:?}", frames, start.elapsed());
        return Ok(());
    }
    ...
    println!("\n== Done. Consumed {} frames + {} stats envelopes in {:?} ==",
             frames, stats_seen, start.elapsed());
    Ok(())
}
...
fn synthetic_frame_summary(seq: u32) -> FrameSummary {
    use android_hid_connect::ai::{MotionVector, ObjectBox, TextRegion,
                                    FLAG_KEYFRAME, FLAG_MOTION, FLAG_OBJECTS, FLAG_SCENE_CHANGE};
    let t = seq as f32 * 0.1;
    let features: Vec<f32> = (0..64).map(|i| {
        let v = ((t + i as f32) * 0.13).sin().abs();
        (v * 1000.0) as u32 as f32 / 1000.0
    }).collect();
    let motion = if seq % 3 == 0 {
        vec![MotionVector { x: 540, y: 1200, dx: (t.sin() * 10.0) as i16, dy: (t.cos() * 8.0) as i16 }]
    } else {
        vec![]
    };
    let objects = if seq % 5 == 0 {
        vec![ObjectBox { x: 100, y: 200, w: 300, h: 80, class_id: 0, confidence: 220 }]
    } else {
        vec![]
    };
    let text_regions = if seq % 7 == 0 {
        vec![TextRegion { x: 50, y: 100, w: 200, h: 50 }]
    } else {
        vec![]
    };
    let mut flags = 0;
    if seq % 10 == 0 { flags |= FLAG_SCENE_CHANGE; }
    if seq == 0 || seq % 10 == 0 { flags |= FLAG_KEYFRAME; }
    if !motion.is_empty() { flags |= FLAG_MOTION; }
    if !objects.is_empty() { flags |= FLAG_OBJECTS; }
    FrameSummary {
