    ...
    while i < bytes.len() {
        ...
        match tag {
            ...
            TAG_TOUCH => {
                ...
                let x = i32::from_be_bytes([bytes[i+10], bytes[i+11], bytes[i+12], bytes[i+13]]);
                let y = i32::from_be_bytes([bytes[i+14], bytes[i+15], bytes[i+16], bytes[i+17]]);
                out.push(TouchFrame { action, pointer_id, x, y });
                i += 32;
            }
            _ => break,
        }
...
fn ten_point_lifecycle_via_inject_touch() {
    ...
    let bytes = s.into_inner().into_bytes();
    let frames = parse_touch_frames(&bytes);
    assert_eq!(frames.len(), 30, "expected 30 touch events, got {}", frames.len());
    for id in 0..10u64 {
    ...
    for i in 0..10 { assert_eq!(frames[i].action, 0, "frame {i} should be DOWN"); }
    for i in 10..20 { assert_eq!(frames[i].action, 2, "frame {i} should be MOVE"); }
    for i in 20..30 { assert_eq!(frames[i].action, 1, "frame {i} should be UP"); }
}
